...
export const getStaticProps = async () => {
  const todayDataGraphFetchResponse = await fetch(
    `${process.env.NEXT_PUBLIC_GRAPH}/today`
  );
  const todayDataGraphFetchJson = await todayDataGraphFetchResponse.json();

  const monthDataGraphFetchResponse = await fetch(
    `${process.env.NEXT_PUBLIC_GRAPH}/month`
  );
  const monthDataGraphFetchJson = await monthDataGraphFetchResponse.json();

  const allTimeDataGraphFetchResponse = await fetch(
    `${process.env.NEXT_PUBLIC_GRAPH}/all-time`
  );
  const allTimeDataGraphFetchJson = await allTimeDataGraphFetchResponse.json();

  return {
    props: {
      todayDataGraphFetchJson,
      // weekDataGraphFetchJson,
      monthDataGraphFetchJson,
      allTimeDataGraphFetchJson,
    },
  };
};
...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    setBarGraphDataToday(todayDataGraphFetchJson);
    // setBarGraphDataWeek(weekDataGraphFetchJson);
    setBarGraphDataMonth(monthDataGraphFetchJson);
    setBarGraphDataAllTime(allTimeDataGraphFetchJson);
  }, [
    unitsInCountToday,
    unitsInCount,
    pendingCountToday,
